...
현재 대시보드(`news_preview` / `news_calendar` / `news_dart`)는 **종목·이벤트 단위**의 카탈리스트 정보만 제공한다. supply / DART / 정책뉴스가 세 개의 섬처럼 분리되어 있어, 사용자가 매번 머릿속에서 "외인이 사는 섹터 + 마침 다가올 이벤트도 있는 섹터" 같은 교차 시그널을 만들어야 한다.

이 페이지는 이미 수집 중인 데이터를 **섹터 단위로 합산·결합**하여 두 가지를 한 화면에 제공한다:
1. **자금흐름 (후행)** — `naver_supply_cache.json` 의 외인·기관 순매수를 섹터별 합산
2. **다가올 카탈리스트 (선행)** — 캘린더의 다가올 이벤트(DART 공시·정책 일정·BOK·NEWS_FUTURE)를 그 섹터 종목과 매핑

이 결합이 핵심이다. 자금흐름만 보면 후행 지표지만, **"외인이 사는 섹터 + 다가올 카탈리스트"** 가 교차하면 카탈리스트 앞에서의 자금 선점 패턴을 잡아 선행 시그널로 변환된다.
...
### 3.4 입력 4: 다가올 이벤트 (기존, 신규 결합)

- 데이터 소스: `calendar_page.build_calendar_events()` 가 이미 통합한 미래 이벤트
  - DART future schedule (유상증자 납입일, 자사주 취득 만료, 분할 기일 등)
  - BOK 통화정책 일정
  - NEWS_FUTURE (정책 본회의·국정감사 등 뉴스에서 추출된 미래 일정)
- 매핑 로직: 각 이벤트가 가진 종목 코드(혹은 inferred stock 리스트)를 섹터로 변환 → 섹터별로 그룹화
  - 이벤트가 종목과 직접 연결된 경우 (DART): 그 종목의 섹터로 매핑
  - 이벤트가 거시·정책인 경우 (BOK / 정책뉴스): 영향 섹터 라벨이 있으면 그것 사용, 없으면 매핑하지 않음 (V1 안전 우선)
- 윈도우: **오늘 ~ +14일** 범위만 표시 (V1)
...
│   ├── group_events_by_sector(calendar_events, sector_map)
│   ├── classify_intensity(net_value_pct) → 5단계 라벨
│   └── render_sector_flow_html(aggregated_flows, events_by_sector)
├── test_sector_flow.py        (신규) 집계 + 이벤트 매핑 단위 테스트
...
[build_pages cron 또는 push]         ▼
    ↓
run_pages.py
    ├── load supply cache + sector cache + KRX listings + calendar events
    ├── sector_flow_page.aggregate_sector_flows(window=1)
    ├── sector_flow_page.aggregate_sector_flows(window=5)
    ├── sector_flow_page.group_events_by_sector(calendar_events, sector_map)
    │     → {"반도체": [event1, event2, ...], "2차전지": [...], ...}
    ├── sector_flow_page.render_sector_flow_html(flows, events_by_sector)
    │     → public/news_sector_flow.html
    └── inject_nav 적용
```
...
│  📅 다가올 카탈리스트 (+14일, 3건)            │
│    5/8  반도체 진흥법 본회의                  │
│    5/12 삼성전자 1Q 실적 (DART)              │
│    5/15 SK하이닉스 자사주 취득 만료 (DART)   │
│  ─────────────────────────                    │
│  📈 매수 TOP3                                 │
│    삼성전자        +850억                     │
│    SK하이닉스      +320억                     │
...
- 카탈리스트가 0건인 섹터: 해당 블록 자체를 숨김 (카드 빈약 방지)
- 카탈리스트가 4건 이상: 상위 3건만 + "외 N건 더" 링크 (캘린더 페이지로 점프)
- 이벤트 종류 라벨: `(DART)` / `(BOK)` / `(정책)` 같은 출처 태그를 우측에 작은 글씨로
...
- `aggregate_sector_flows()` 단위 테스트
  ...
  - 매핑 안 되는 종목 → "기타" 그룹화
  - 종목 수 3개 미만 섹터 처리
- `group_events_by_sector()` 단위 테스트
  - 종목 직접 연결 이벤트 → 그 섹터로 매핑
  - 영향 섹터 라벨이 있는 거시 이벤트 → 해당 섹터로 매핑
  - 매핑 불가 이벤트 → 어떤 섹터에도 표시 안 됨 (V1 안전 우선)
  - +14일 윈도우 외 이벤트 제외
- `classify_intensity()` 경계값 테스트 (5단계)
- 성능: 200종목 × 28섹터 합산 + 이벤트 매핑이 200ms 이내
...
- **카탈리스트 결합 (옵션 1)**: 자금흐름 단독은 후행 지표라는 한계. 다가올 이벤트와 결합하면 "카탈리스트 앞에서의 자금 선점" 패턴을 잡아 선행 시그널로 변환 가능. 추가 외부 API 0개로 구현 가능.
- **거시 이벤트 영향 섹터 매핑 V1 보수적 처리**: BOK/정책뉴스처럼 종목과 직접 연결 안 된 이벤트는 영향 섹터 라벨이 명시적으로 있을 때만 매핑. V1 에서는 "잘못된 섹터에 잘못된 이벤트가 붙는 노이즈" 가 더 큰 비용.
